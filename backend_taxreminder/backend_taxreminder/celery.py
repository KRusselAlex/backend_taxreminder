...
app.conf.beat_schedule = {
    ...
    'send-salaried-reminders': {
        ...
        'schedule': crontab(minute=0, hour=0, day_of_month=31, month_of_year=12),  # 31 December
    },
    'send-salaried-relances': {
        'task': 'notifications.tasks.send_salaried_relances',
        'schedule': crontab(minute=0, hour=0, day_of_month=31, month_of_year=1),  # 31 January
    },
      'send-salaried-february-relances': {
        'task': 'notifications.tasks.send_salaried_final_relances',
        'schedule': crontab(minute=0, hour=0, day_of_month='28', month_of_year=2),  # february relances
    },

    'send-salaried-final-relances': {
        'task': 'notifications.tasks.send_salaried_final_relances',
        'schedule': crontab(minute=0, hour=0, day_of_month='10,20,25,31', month_of_year=3),  # March relances
    },

    # Reminder tasks for companies
       'send-company-reminders-january': {
        'task': 'notifications.tasks.send_company_reminders',
        'schedule': crontab(minute=0, hour=0, day_of_month="1,15", month_of_year=1),  # 1er et 15 janvier
    },

    # Rappels pour les déclarations des mois de janvier, février et mars
    'send-company-reminders-april': {
        'task': 'notifications.tasks.send_company_reminders',
        'schedule': crontab(minute=0, hour=0, day_of_month="1,15", month_of_year=4),  # 1er et 15 avril
    },

    # Rappels pour les déclarations des mois d'avril, mai et juin
    'send-company-reminders-july': {
        'task': 'notifications.tasks.send_company_reminders',
        'schedule': crontab(minute=0, hour=0, day_of_month="1,15", month_of_year=7),  # 1er et 15 juillet
    },

    # Rappels pour les déclarations des mois juillet, août et septembre
    'send-company-reminders-october': {
        'task': 'notifications.tasks.send_company_reminders',
        'schedule': crontab(minute=0, hour=0, day_of_month="1,15", month_of_year=10),  # 1er et 15 october
    },
    ...
    'reset-report-sent-company': {
        ...
        'schedule': crontab(minute=0, hour=0),  # Cette tâche s'exécute chaque jour à minuit
    }
}
